	...
	<section class="container middle">
		...
		<table id="cart-table">
				...
				<c:forEach items="${shoppingCart.items }" var="item">
					<tr>
						...
						<td class="quantity-input-cell"><input type="number" min="0"
							...
							value="${shoppingCart.getQuantities(item)}" /></td>
						<td class="numeric-cell">${shoppingCart.getTotal(item) }</td>
						<td class="remove-item">
							<form
								action="${s:mvcUrl('SCC#remove').arg(0, item.product.id).arg(1, item.typePrice).build()}"
								method="POST">
								<input type="image"
			...
			<tfoot>
				<tr>
					<td colspan="3">
						<form action="${s:mvcUrl('PC#finalizeBuyItem').build()}"
							method="POST">
